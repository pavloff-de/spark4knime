...
import static de.pavloff.spark4knime.jsnippet.JavaSnippetDocument.GUARDED_BODY_END;
import static de.pavloff.spark4knime.jsnippet.JavaSnippetDocument.GUARDED_BODY_START;
import static de.pavloff.spark4knime.jsnippet.JavaSnippetDocument.GUARDED_FIELDS;
import static de.pavloff.spark4knime.jsnippet.JavaSnippetDocument.GUARDED_IMPORTS;
...
import de.pavloff.spark4knime.jsnippet.expression.Abort;
import de.pavloff.spark4knime.jsnippet.expression.AbstractJSnippet;
import de.pavloff.spark4knime.jsnippet.expression.Cell;
import de.pavloff.spark4knime.jsnippet.expression.ColumnException;
import de.pavloff.spark4knime.jsnippet.expression.FlowVariableException;
import de.pavloff.spark4knime.jsnippet.expression.Type;
...
import de.pavloff.spark4knime.jsnippet.guarded.GuardedSection;
 ...
 * jar-files to be included or by changing the contents of the snippets
...
public final class JavaSnippet {
	 ...
	 * @throws IOException
	 *             when a file could not be loaded
	 */
	...
	/**
	 * Return true when this snippet is the creator and maintainer of the given
	 * source.
	 * 
	 * @param source
	 *            the source
	...
	private static DefaultMutableTreeNode getChild(
			final DefaultMutableTreeNode curr, final String p) {
		for (int i = 0; i < curr.getChildCount(); i++) {
			DefaultMutableTreeNode child = (DefaultMutableTreeNode) curr
					.getChildAt(i);
	...
	/**
	 * Initialize GUARDED_IMPORTS and GUARDED_FIELDS with information from the
	 * settings.
	 */
	...
	/**
	 * Get the list of default imports. Override this method to append or modify
	 * this list.
	 * 
	 * @return the list of default imports
	 */
	protected String[] getSystemImports() {
		String pkg = "de.pavloff.spark4knime.jsnippet.expression";
		return new String[] { AbstractJSnippet.class.getName(),
				Abort.class.getName(), Cell.class.getName(),
				ColumnException.class.getName(), TypeException.class.getName(),
				"static " + pkg + ".Type.*", "java.util.Date",
				"java.util.Calendar", "org.w3c.dom.Document",
				"java.io.Serializable" };
	}

	/**
	 * Validate settings which is typically called in the configure method of a
	 * node.
	 * 
	 * @param spec
	...
	public ValidationReport validateSettings(final DataTableSpec spec,
			...
			if (index >= 0) {
				...
				if (!colSpec.getType().equals(field.getKnimeType())) {
					...
					DataValueToJava converter = provider.getDataValueToJava(
							colSpec.getType(), field.getJavaType().isArray());
					if (converter.canProvideJavaType(field.getJavaType())) {
						...
						errors.add("The type of the column \""
								+ field.getKnimeName() + "\" has changed.");
					}
				}
			} else {
		...
		for (InVar field : m_fields.getInVarFields()) {
			FlowVariable var = flowVariableRepository.getFlowVariable(field
					.getKnimeName());
			if (var != null) {
		...
		for (OutCol field : m_fields.getOutColFields()) {
			...
			if (field.getReplaceExisting() && index < 0) {
				errors.add("The output column \"" + field.getKnimeName()
		...
		for (OutVar field : m_fields.getOutVarFields()) {
			...
			if (field.getReplaceExisting() && var == null) {
				errors.add("The output flow variable \"" + field.getKnimeName()
						+ "\" is marked to be a replacement, "
						+ "but an input with this name does not exist.");
			}
			if (!field.getReplaceExisting() && var != null) {
	...
	/**
	 * The execution method when no input table is present. I.e. used by the
	 * java edit variable node.
	 * 
	 * @param flowVariableRepository
	 *            flow variables at the input
	 * @param exec
	 *            the execution context to report progress, may be null when
	...
	public void execute(final FlowVariableRepository flowVariableRepository,
		...
		factory.getCells(new DefaultRow(RowKey.createRowKey(0), new DataCell[0]));
	}
	...
	/**
	 * Set the list of additional jar files to be added to the class path of the
	 * snippet.
	 * 
	 * @param jarFiles
	 *            the jar files
	...
	private Class<? extends AbstractJSnippet> createSnippetClass() {
		...
		if (success) {
			...
			for (Diagnostic<? extends JavaFileObject> d : digsCollector
					.getDiagnostics()) {
				boolean isSnippet = this.isSnippetSource(d.getSource());
				if (isSnippet
						&& d.getKind()
								.equals(javax.tools.Diagnostic.Kind.ERROR)) {
					msg.append("Error: ");
