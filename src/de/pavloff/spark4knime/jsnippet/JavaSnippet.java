...
public final class JavaSnippet {
    ...
    private File createJSnippetJarForSpark(Class<? extends AbstractJSnippet>... snippet) 
    		throws IOException {
    	m_snippetJarFile = new File(m_tempClassPathDir, "jsnippetForSpark.jar");
        JarOutputStream jar = new JarOutputStream(new FileOutputStream(m_snippetJarFile));

        try {
        	for (Object o : snippet) {
	            Class<?> cl = (Class<?>)o;
	            String className = cl.getName();
	            JarEntry entry = new JarEntry(className.concat(".class"));
    ...
    private Class<? extends AbstractJSnippet> createSnippetClass() {
        ...
        if (success) {
            try {
                ...
                Class<? extends AbstractJSnippet> snippet = (Class<? extends AbstractJSnippet>)
